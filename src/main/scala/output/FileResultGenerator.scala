...
class FileResultGenerator(filename: String) extends ResultGenerator{
  ...
  override def write(meetings: List[Meeting]): Unit = {
    ...
    meetings.map(m => {
      bw.write(s"${new DateTime(m.timestamp).toString("yyyy-MM-dd'T'HH:mm:ss")},${m.uid1},${m.location1.x},${m.location1.y},${m.uid2},${m.location2.x},${m.location2.y},${m.location1.floor}\n")
    })
    bw.close()
  }
